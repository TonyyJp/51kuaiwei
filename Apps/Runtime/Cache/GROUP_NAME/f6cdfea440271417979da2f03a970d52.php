    ...
    <script>
        ...
        <div class="right-content fRight">
            ...
            <div class="mainbody mt10">
                <form method='post' id="form1" name="form1" action="<?php echo U('Member/alipayapi');?>">
                    <div style='border-bottom: 1px dashed #47B717;'>
                        <label>账户余额：</label>
                        <span class="star" style="font-size: 22px;font-family: Georgia,Arial;font-weight: 700;}"><?php echo getMemberAmount(session('id'),session('account'));?></span><span>元</span>
                    </div>
                    <div style="margin-top:10px;">
                        <label>充值金额：</label>
                        <input type="text" name="WIDprice" class="ipt2">
                        <span class="star">*</span> <span class="tip">请输入充值金额</span>
                    </div>
                    <div style="margin-top:10px;">
                        <label>充值方式：</label>
                        <?php if(isset($paytypelist)): if(is_array($paytypelist)): $i = 0; $__LIST__ = $paytypelist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$paytypevo): $mod = ($i % 2 );++$i;?><input type="radio" name="WIDtype" value="<?php echo ($paytypevo["id"]); ?>" checked=true style="height:24px;line-height: 24px;" />
                                <span><?php echo ($paytypevo["payname"]); ?></span><?php endforeach; endif; else: echo "" ;endif; ?>
                            <?php else: ?>
                            <span>本站暂未开启在线支付功能，如需帮助请联系管理员。</span><?php endif; ?>
                        <span class="star">*</span> <span class="tip"></span>
                    </div>
                    <div style="margin-top:10px;">
                        <label>备注：</label>
                        <textarea name="WIDbody" />
                        </textarea>
                    </div>
                    <div style="margin-top:10px;">
                        <label>验证码：</label>
                        <input type="text" style="width: 95px;" id="verify" name="verify" class="ipt3">
                        <img id="verifyImg" src="<?php echo U('Member/verify');?>" onClick="fleshVerify()" border="0" alt="点击刷新验证码" style="cursor:pointer" align="absmiddle"></div>
                    <div style="margin-top:10px;">
                        <label>&nbsp;</label>
