...
<div class="hy mt5 clear">
  <div class="w1000 wrap">
    <div class="c-content command">
      <div class="c-top">
        <p>
          我的位置：<a href="">首页</a> &gt; <span class="b_nav"><?php if(is_array($position)): $i = 0; $__LIST__ = $position;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><a href="<?php echo U('Weixin/index',array('id'=>$vo['id']));?>"><?php echo ($vo["catname"]); ?></a> &gt;<?php endforeach; endif; else: echo "" ;endif; ?>
        </p>
      </div>
      <div class="c-left fLeft">
        <div class="box1">
          ...
          <div class="fLeft b1">
            <p>
                <img src="<?php if(empty($data["qrcode"])): ?>../Public/images/nopic.gif<?php else: ?>__ROOT__/Uploads<?php echo ($data["qrcode"]); endif; ?>" alt="<?php echo ($data["pubaccount"]); ?>" width="250" />            </p>
            <p class="flip">
	    ...
	    <?php if(!empty($nextdata["id"])): ?><a class="fixed-btn" rel="next" href="<?php echo U('Weixin/show',array('id'=>$nextdata['id']));?>" title="<?php echo ($nextdata["pubaccount"]); ?>"><i class="next"></i>下一篇</a><?php endif; ?>
          ...
          <div class="fRight b2">
            ...
            <p><strong>手机号：</strong><?php echo ($data["phone"]); ?></p>
            <p><strong>QQ号：</strong><?php echo ($data["qq"]); ?></p>
            <p><strong>关注度：</strong><span class="STYLE1"><?php echo ($data["hits"]); ?>人关注</span></p>
            <p><strong>评价度：</strong><img alt="星级" src="http://www.75820.com/Apps/Tpl/weixinqun/Public/images/xx5.jpg" /></p>
            <p>
              <strong>地区：</strong><?php echo (getareasname($data["province"])); ?>/<?php echo (getareasname($data["city"])); ?></p>
            <p>
          ...
          <ul>
            <?php $_result=M('Weixin')->where('status=1 and catid in ('.$data['catid'].')')->order('rand()')->limit(15)->select();foreach($_result as $key=>$weixin):?><li>
              <a href="<?php echo U('Weixin/show',array('id'=>$weixin['id']));?>" title="<?php echo ($weixin["pubaccount"]); ?>" target='_blank'>
                <img src="<?php if(empty($weixin["logo"])): if(empty($weixin["weblogo"])): ?>../Public/images/nopic.gif<?php else: echo ($weixin["weblogo"]); endif; else: ?>__ROOT__/Uploads<?php echo ($weixin["logo"]); endif; ?>" width="80" height="80">
              </a>
              <p><a href="<?php echo U('Weixin/show',array('id'=>$weixin['id']));?>" title="<?php echo ($weixin["pubaccount"]); ?>" target='_blank'><?php echo (msubstr($weixin["pubaccount"],0,5)); ?></a></p>
            </li><?php endforeach;?>
          </ul>
        </div>
       <div class="comment">
