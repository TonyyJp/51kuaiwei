...
class MemberAction extends CommonAction {
    ...
    public function alipayapi() {
        ...
        /* *************************请求参数************************* */
        //支付类型
        $payment_type = $alipay_config['payid'];
        //必填，不能修改
        //服务器异步通知页面路径
...
//        $this->checkUser();
    ...
    public function add() {
        ...
        if (IS_POST) {
            ...
            $list = $model->add();
            if ($list !== false) { //保存成功
                $this->success('提交成功!', U('Member/manage?publish_type_id=' . $_POST['publish_type_id']));
            } else {
                //失败提示
                $this->error('提交失败!');
            }
            ...
            //获取下级子分类
            $childCatMap['pid'] = array('eq', $publish_type_id);
