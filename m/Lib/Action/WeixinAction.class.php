...
class WeixinAction extends CommonAction{
    ...
    */
	...
	public function huoyuanshow()
	{
        ...
        $category = M('category') -> where("id = {$da['catid']}") -> find();
        $category1  = M('category') -> where("id = {$category['pid']}") -> find();

		//上一条
		$this -> assign('beforeda',D('weixin') -> where("id > '{$id}' and publish_type_id=1") -> order("id asc ") -> limit(1)->find());
		//xia一条
		$this -> assign('afterda',D('weixin') -> where("id < '{$id}' and publish_type_id=1") -> order("id desc ") -> limit(1)->find());
		        $position=D('Common')->getPosition($id);
        foreach ($position as $value) {
            $title=$value['catname']."_".$title;
        }

        $this -> daohang = "<a href='index.php'>首页</a> > {$category1['catname']} > {$category['catname']} > {$da['pubaccount']}";


        $title=  substr($title, 0, strlen($title)-1);
        $this->seo(($da['title'])?$da['title']:$title, ($da['keywords'])?$da['keywords']:C(SITE_KEYWORDS), ($da['description'])?$da['description']:C(SITE_DESCRIPTION), $position);
